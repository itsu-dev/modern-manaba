...
window.onload = () => {
  ...
  if (document.getElementById("mylinks")) {
    ...
    document
      ...
      .insertBefore(options, document.getElementById("mylinks-logout"));
    ...
    document
      .getElementById("mylinks")
      .insertBefore(sep, document.getElementById("mylinks-logout"));
    }
  ...
  div2.setAttribute(
    "onclick",
    "window.location.href = 'home_library_reminder';"
  );
  ...
  responDiv.setAttribute(
    "onclick",
    "window.location.href = 'https://atmnb.tsukuba.ac.jp/attend/tsukuba?relogin=1';"
  );
  ...
  if (regex.test(location.pathname)) {
    if (document.getElementsByClassName("pageheader-course")[0])
    ...
    if (document.getElementsByClassName("pageheader-course-extlink").length) {
      document
        .getElementsByClassName("pageheader-course-courseteacher")[0]
        .appendChild(
          document.getElementsByClassName("pageheader-course-extlink")[0]
        );
      }
    ...
    if (document.getElementsByClassName("courseheader-courselinklist").length) {
      document
        .getElementsByClassName("pageheader-course-courseteacher")[0]
        .appendChild(
          document.getElementsByClassName("courseheader-courselinklist")[0]
        );
      }
  ...
  const examPageRegex = new RegExp(
    "https://manaba.tsukuba.ac.jp/ct/home_library_query|https://manaba.tsukuba.ac.jp/ct/course_[0-9]+_(query|survey|report)"
  );
  // Manaba Enhancedがインストールされていて、提出期限によって色付けされるページなら
  if (
    document.getElementsByClassName("mylinks-sep").length === 4 &&
    examPageRegex.test(url)
  ) {
    // テーブルの背景色の変更を無効化する
